...
cnn_train = pd.read_csv('../data/insample/scores/cnn_3.csv')
cnn_score = pd.read_csv('../data/outsample/scores/cnn_3.csv')
cnn_feat = cnn_train.append(cnn_score).rename(columns={'deal_probability':'cnn_model'})
train_data = train_data.merge(cnn_feat, on='item_id')
test_data = test_data.merge(cnn_feat, on='item_id')

vgg_train = pd.read_csv('../data/data/features/image/vgg_train.csv')
vgg_score = pd.read_csv('../data/data/features/image/vgg_test.csv')
vgg_feat = vgg_train.append(vgg_score).rename(columns={'deal_probability':'vgg_model'})
train_data = train_data.merge(vgg_feat, on='item_id', how='left')
test_data = test_data.merge(vgg_feat, on='item_id', how='left')

xcept_train = pd.read_csv('../data/data/features/image/xception_train.csv')
xcept_score = pd.read_csv('../data/data/features/image/xception_test.csv')
xcept_feat = xcept_train.append(xcept_score).rename(columns={'deal_probability':'xcept_model'})
train_data = train_data.merge(vgg_feat, on='item_id', how='left')
test_data = test_data.merge(vgg_feat, on='item_id', how='left')
...
train_data.to_csv('../data/data/lightgbm/train_data_3.csv', index=False)
test_data.to_csv('../data/data/lightgbm/test_data_3.csv', index=False)
...
for i in range(5):
    ...
    train_sub.to_csv('../data/data/lightgbm/dataset/train_3_{}.csv'.format(i), index=False)
    valid_sub.to_csv('../data/data/lightgbm/dataset/valid_3_{}.csv'.format(i), index=False)