...
def execute(mode):
    ...
    model = define_model()
    model.load_weights('../data/data/neural/model_1/weights_{}.h5'.format(mode))
    ...
    scores.to_csv('../data/data/neural/score_1/valid_{}.csv'.format(mode), index=False)
...
def execute(mode):
    ...
    scores = pd.DataFrame(scores, columns=['deal_probability'])
    scores = driver.join(scores)
    scores.to_csv('../data/data/neural/score_1/test_{}.csv'.format(mode), index=False)
    print('{} mode scoring finished...'.format(mode))
    return None
...
for i in range(5):
    i += 1
    temp = pd.read_csv('../data/data/neural/score_1/valid_{}.csv'.format(i))
    dataset = dataset.append(temp)

dataset.to_csv('../data/insample/scores/cnn_1.csv', index=False)
...
for i in range(5):
    i += 1
    temp = pd.read_csv('../data/data/neural/score_1/test_{}.csv'.format(i))
    dataset = dataset.append(temp)

dataset = dataset.groupby(['item_id'])['deal_probability'].mean().reset_index()
dataset.to_csv('../data/outsample/scores/cnn_1.csv', index=False)