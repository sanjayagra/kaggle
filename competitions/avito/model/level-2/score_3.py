...
def score(mode):
    model = lgb.Booster(model_file='../data/data/lightgbm/model_3/lightgbm_{}.model'.format(mode))
    ...
    test_data = pd.read_csv('../data/data/lightgbm/test_data_3.csv'.format(mode))
    ...
    valid_score.to_csv('../data/data/lightgbm/score_3/valid_{}.csv'.format(mode), index=False)
    test_score.to_csv('../data/data/lightgbm/score_3/test_{}.csv'.format(mode), index=False)
    return None
...
for i in range(5):
    i += 1
    temp = pd.read_csv('../data/data/lightgbm/score_3/valid_{}.csv'.format(i))
    dataset = dataset.append(temp)

dataset.to_csv('../data/insample/scores/lightgbm_3.csv', index=False)
...
for i in range(5):
    i += 1
    temp = pd.read_csv('../data/data/lightgbm/score_3/test_{}.csv'.format(i))
    dataset = dataset.append(temp)

dataset = dataset.groupby(['item_id'])['deal_probability'].mean().reset_index()
dataset.to_csv('../data/outsample/scores/lightgbm_3.csv', index=False)