...
data = pd.read_csv('../data/data/image/train_data.csv')
generator = DataGenerator(data, 32)
model = define_model()
model.load_weights('../data/data/image/model/weights_1.h5')
...
scores = model.predict_generator(**params)
scores = pd.DataFrame(scores, columns=['deal_probability'])
driver = data[['item_id']].join(scores)
driver.to_csv('../data/vgg_train.csv', index=False)