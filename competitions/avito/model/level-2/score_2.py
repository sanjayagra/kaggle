...
def score(mode):
    model = lgb.Booster(model_file='../data/data/lightgbm/model_2/lightgbm_{}.model'.format(mode))
    valid_data = pd.read_csv('../data/data/lightgbm/dataset/valid_2_{}.csv'.format(mode))
    valid_driver = valid_data[['item_id']].copy()
    valid_data = valid_data.drop(['item_id','deal_probability'], axis=1)
    test_data = pd.read_csv('../data/data/lightgbm/test_data_2.csv'.format(mode))
    ...
    valid_score.to_csv('../data/data/lightgbm/score_2/valid_{}.csv'.format(mode), index=False)
    test_score.to_csv('../data/data/lightgbm/score_2/test_{}.csv'.format(mode), index=False)
    return None
...
for i in range(5):
    i += 1
    temp = pd.read_csv('../data/data/lightgbm/score_2/valid_{}.csv'.format(i))
    dataset = dataset.append(temp)

dataset.to_csv('../data/insample/scores/lightgbm_2.csv', index=False)
...
for i in range(5):
    i += 1
    temp = pd.read_csv('../data/data/lightgbm/score_2/test_{}.csv'.format(i))
    dataset = dataset.append(temp)

dataset = dataset.groupby(['item_id'])['deal_probability'].mean().reset_index()
dataset.to_csv('../data/outsample/scores/lightgbm_2.csv', index=False)