...
def importance(model, feats):
    ...
    importance.to_csv('../data/data/lightgbm/importance_2.csv', index=False)
    return None
    
def execute(mode, imp=False):
    train_data = pd.read_csv('../data/data/lightgbm/dataset/train_2_{}.csv'.format(mode))
    ...
    model.save_model('../data/data/lightgbm/model_2/lightgbm_{}.model'.format(mode))
    if imp:
