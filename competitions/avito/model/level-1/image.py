...
def merge(file, name):
    ...
    path = '../data/data/features/image/' + file
    file = pd.read_csv(path)
    file = file.rename(columns={'deal_probability':name})
    actual = actual.merge(file, on='item_id', how='inner')
    actual[name] = actual[name].fillna(0)
    features += [name]
    rmse = np.sqrt(mean_squared_error(actual['actual'], actual[name]))
    print('rmse:', round(rmse,5))
    return None

merge('vgg_train.csv', 'model_1')
merge('xception_train.csv', 'model_2')
merge('vgg_train_1.csv', 'model_3')
...
def merge(file, name):
    ...
    file = pd.read_csv(path)
    file = file.rename(columns={'deal_probability':name})
    score = score.merge(file, on='item_id',how='inner')
    score[name] = score[name].fillna(0.)
    return None

merge('vgg_test.csv', 'model_1')
merge('xception_test.csv', 'model_2')
merge('vgg_test_1.csv', 'model_3')
...
train_data.to_csv('../data/data/features/image/image_train.csv', index=False)
test_data.to_csv('../data/data/features/image/image_test.csv', index=False)