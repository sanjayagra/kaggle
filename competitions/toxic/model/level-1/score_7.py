...
def score_model(mode):
    ...
    score_text = pd.read_csv('../data/data/source_2/score/score_data.csv')
    ...
    scores = model.predict(score_text, batch_size=2048)
...
submit.to_csv('../data/submit/model_8.csv', index=False)

def score_model(mode):
    train_text = pd.read_csv('../data/data/source_2/train/train_data_{}.csv'.format(mode))
    valid_text = pd.read_csv('../data/data/source_2/train/test_data_{}.csv'.format(mode))
    score_text = pd.read_csv('../data/data/source_2/train/test_data_{}.csv'.format(mode))
    labels = pd.read_csv('../data/data/source_2/train/test_labels_{}.csv'.format(mode))
    score_data = score_text[['id']]
    score_text, embedding_matrix = dataflow(train_text, valid_text, score_text)
    model = define_model(embedding_matrix)
    path = './weights/model_{}-Best.h5'.format(mode)
...
submit.to_csv('../data/model/model_8.csv', index=False)
