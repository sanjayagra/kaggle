...
def callbacks(suffix):
    ...
    logger = CSVLogger('../data/data/source_4/model_1/logger_{}.log'.format(suffix))
    return snap + [stop, logger]

def execute(mode):
    mode += 1 
    train_text = pd.read_csv('../data/data/source_4/train/train_data_{}.csv'.format(mode))
    train_label = pd.read_csv('../data/data/source_4/train/train_labels_{}.csv'.format(mode))
    valid_text = pd.read_csv('../data/data/source_4/train/test_data_{}.csv'.format(mode))
    valid_label = pd.read_csv('../data/data/source_4/train/test_labels_{}.csv'.format(mode))
